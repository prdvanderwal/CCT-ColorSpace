"""
Obtained from https://github.com/nis-research/afa-augment/blob/main/config_utils.py
"""
...
def make_config(
        ...
        epochs, grad_clip, batch_size, learning_rate, colorspace,
        dset, model_arch,
    ...
    config.dataset = dset
    config.data_dir = os.path.join('src', 'data')
...
class ConfigBuilder:
    ...
    def build(
            ...
            use_mix=False, mean_str=None, min_str=None, orth=True, premix='none', colorspace=None
    ):
        _lookup_ds = ds.upper()

        ################################# Added for DL #########################################
        if colorspace:
            _lookup_ds = f'{_lookup_ds}_{colorspace}'
        ########################################################################################
